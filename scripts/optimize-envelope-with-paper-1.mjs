...
const sourceFile = path.join(projectRoot, 'src/assets/images/envelope/envelope_with_paper_1.png');
...
async function main() {
  ...
  console.log('Processing envelope_with_paper_1.png...\n');
  
  const variants = [];
  
  for (const { width, suffix } of BACKGROUND_SIZES) {
    const outputFilename = `envelope_with_paper_1-${suffix}.webp`;
  ...
  const newEntry = {
    original: "envelope/envelope_with_paper_1.png",
